...
public class ActionService {
    ...
    public ActionResponse getAllAction(
        ...
        if(device.equals("all")&&state.equals("all")){
            actionPage= actionRepo.getAllAction(startDate,endDate,pageable);
        }
        if(!device.equals("all")&&!state.equals("all")){
            ActionEnum statee = state.equals("on")?ActionEnum.on:ActionEnum.off;
            actionPage = actionRepo.
                    findAllByTimeBetweenAndDeviceNameEqualsIgnoreCaseAndActionEquals
                            (startDate,endDate,device,statee,pageable);
        }
        if(!device.equals("all")&&state.equals("all")){
            actionPage = actionRepo.findAllByTimeBetweenAndDeviceNameEqualsIgnoreCase(startDate,endDate,device,pageable);
        }
        if(device.equals("all")&&!state.equals("all")){
            ActionEnum statee = state.equals("on")?ActionEnum.on:ActionEnum.off;
            actionPage = actionRepo.findAllByTimeBetweenAndActionEquals(startDate,endDate,statee, pageable);
        }
